...
require_once $_SERVER["DOCUMENT_ROOT"].'/_h5ai/private/php/ext/StreamZipArchive.php';
class Api {
    ...
    private function on_download() {
        ...
        // 判断是否为stream-zip下载类型，如果是就调用自己写的zip流下载输出

        if("stream-zip" === $type) {
            $archive = new Archive($this->context);
            $ok = $archive->streamzipoutput($as, $type, $base_href, $hrefs);
        }else {
            $archive = new Archive($this->context);
            set_time_limit(0);
            session_write_close();
